...
class ParticleSystem:
    def __init__(self, resolution):
        ...
        self.bound = np.array(resolution) / self.screen_to_world_ratio
        ...
        self.m_V = 0.8 * self.particle_diameter**self.dim  # Volume
        self.particle_max_num = 2**15
        self.particle_max_num_per_cell = 100
        self.particle_max_num_neighbor = 100
        self.particle_num = ti.field(dtype=int, shape=())

        # Grid information
        self.grid_size = self.support_radius
        self.grid_num = np.ceil(np.array(resolution)/self.grid_size).astype(int)
        self.grid_particles_num = ti.field(dtype=int)
        self.grid_particles = ti.field(dtype=int)
        self.padding = self.grid_size

        # Particle information
        self.x = ti.Vector.field(n=self.dim, dtype=float)
        self.v = ti.Vector.field(n=self.dim, dtype=float)
        ...
        self.particle_neighbors_num = ti.field(dtype=int)
        self.particle_neighbors = ti.field(dtype=int)
        ...
        # To initialize cell, we need another more dimension 
    ...
    def pos_to_index(self, pos):
        # Check which grid the current particle belongs to
